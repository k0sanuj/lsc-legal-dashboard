...
Existing shareholdings expose their record IDs and correction forms in the entity page. Slack entity detail returns the same ownership IDs, fields and source references for the shared update operation. Profiles and filings can also be edited; these registers do not provide destructive delete operations.
...
Existing schedules expose title, owner, source and cadence edits on the page and through Slack. Edits require the exact displayed `updated_at` revision and reject stale submissions. Review kind, linked source and original start stay fixed. A cadence change retains already due reviews, completion evidence and dependency work, replaces only future open scheduled tasks, and applies the new rule after the recorded `cadence_effective_from` date. Confirming a later public cadence does not create retrospective obligations. Serializable schedule transactions prevent an overlapping materializer from restoring obsolete future tasks.
...
- `python3 /tmp/legal-v2-test-runtime.py npx tsx scripts/verify-review-schedule-edit.ts`: isolated schedule edits, stale revision and invalid cadence rejection, retention of due/completed/dependency history, replacement of future open scheduled work, and repeated materialization after a later public cadence is supplied.
