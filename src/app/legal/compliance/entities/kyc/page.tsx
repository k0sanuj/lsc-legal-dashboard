...
export default async function KycPage({
  ...
  const profileFilter = typeof params.profile === "string" ? params.profile : undefined
  const where: Prisma.KycDocumentWhereInput = {}
  if (profileFilter) where.entity_profile_id = profileFilter
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <Link href="/legal/compliance/entities" className="text-sm text-primary underline">Entities</Link>
          <h1 className="text-2xl font-bold tracking-tight">KYC register</h1>
          <p className="text-muted-foreground">
            Know Your Customer document management across entities and
            jurisdictions
          </p>
        </div>
        <CreateKycForm entityProfileId={profileFilter} />
      </div>
      ...
      <Card>
        <CardContent className="pt-4">
          <form className="flex flex-wrap items-center gap-3">
            {profileFilter && <input type="hidden" name="profile" value={profileFilter} />}
            ...
            <Link
              href="/legal/compliance/entities/kyc"
