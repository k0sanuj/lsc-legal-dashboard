...
import type { LifecycleStatus, Entity, Prisma } from "@/generated/prisma/client"

function getDocuments(where: Prisma.LegalDocumentWhereInput) {
...
export default async function DocumentsListPage({
  ...
  try {
    documents = await getDocuments(where)
  } catch {
    loadError = true
  }

  if (loadError) {
    return <DocumentsLoadError />
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Search & Filter Bar */}
      <form action="/legal/documents" className="flex flex-wrap items-center gap-3">
        ...
        <select
          ...
          className="h-8 w-[180px] rounded-lg border border-input bg-background px-2.5 text-sm"
        >
        ...
        {(search || statusFilter || entityFilter) && (
          <Link
            ...
            className="flex h-8 items-center rounded-lg px-3 text-sm text-muted-foreground hover:text-foreground"
          >
            Clear
          </Link>
        )}
