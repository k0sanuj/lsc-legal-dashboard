...
export async function materializeReviewTasks(now = new Date()) {
  const schedules = await prisma.reviewSchedule.findMany({ where: { active: true } })
  let created = 0
  for (const schedule of schedules) {
    const result = await prisma.reviewTask.createMany({ data: reviewDueDates(schedule, now).map((due) => ({
...
export async function completeReviewTask(session: SessionPayload, taskId: string, evidence: string) {
  ...
  return { success: true, alreadyCompleted: result.count === 0 }
}
