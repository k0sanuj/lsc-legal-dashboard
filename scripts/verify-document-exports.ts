...
async function main() {
  ...
  try {
    ...
    await prisma.legalDocument.update({ where: { id: doc.id }, data: { signature_provider: "opensign", signature_status: "SIGNED", signature_completed_at: new Date(), signature_certificate_status: "unavailable", signature_certificate_error: "OpenSign has not supplied CertificateUrl" } })
    const noCertificate = await queueDocumentExport(session); jobs.push(noCertificate.id)
    await runPendingDocumentExports(1, io, noCertificate.id)
    const certificateGap = await prisma.documentExport.findUniqueOrThrow({ where: { id: noCertificate.id } })
    assert.equal(certificateGap.status, "partial")
    assert.match(JSON.stringify(certificateGap.manifest), /CertificateUrl/)
    await recordArtifact({ documentId: doc.id, stage: "certificate", fileUrl: "https://storage.example.test/certificate", originalName: "certificate.pdf", mimeType: "application/pdf", bytes, actorId: "synthetic-verifier", finalized: true })
    const certificateComplete = await queueDocumentExport(session); jobs.push(certificateComplete.id)
    await runPendingDocumentExports(1, io, certificateComplete.id)
    assert.equal((await prisma.documentExport.findUniqueOrThrow({ where: { id: certificateComplete.id } })).status, "complete")
